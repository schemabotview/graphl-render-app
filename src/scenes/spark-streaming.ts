...
// The WHOLE Spark STRUCTURED STREAMING API on one 16:9 wall — ported from graphl-ux/
// src/scenes/spark-streaming.ts (itself from NodeMap). Module 08 rides this scene,
// spotlighting its region per section via the manifest `focus`/`highlight`. Same node
// ids as the source (e.g. `s-df-water`, `s-trigger`) so the module wiring transfers.
//
// Port transforms vs. graphl-ux (mirror the spark-batch-api port):
//   - `wgrid` imported from patterns.ts (graphl-ux inlined it).
//   - graphl-ux `kind: 'label'` leaves (the dense streaming DataFrame lane) → `term`
//     chips (this engine has no bare 'label' kind).
//   - I/O endpoint `comp()` nodes gain icons (stream/file/database/memory/…).
//   - SceneSpec drops `topic`/`subtitle`; edges are already uniform (no per-edge color).

const G = 0.18
const P = 0.22
// Tight inner padding for the single-column rail containers (sources / read / df /
// sinks / output rails) so each long label fills the box width.
const PT = 0.01

/** A filled leaf chip — the text IS the value (`term`). Default for the option groups. */
const chip = (id: string, label: string, color: string): NodeSeed => ({ id, label, color, kind: 'term', cell: [0, 0] })

/** Was graphl-ux `kind: 'label'` (bare text) — rendered here as a `term` chip. */
const lbl = (id: string, label: string, color: string): NodeSeed => chip(id, label, color)

/** A COMPONENT leaf — icon glyph + label (`symbol`). Used for the I/O endpoints. */
...
const sources = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1, 1], gap: G, padding: PT }, [
    { node: comp('s-src-rate', 'rate', TEAL, 'clock'), at: [0, 0] },
    { node: comp('s-src-ratepm', 'rate-per-microbatch', TEAL, 'clock'), at: [0, 1] },
    { node: comp('s-src-socket', 'socket', TEAL, 'plug'), at: [0, 2] },
    { node: comp('s-src-file', 'file', TEAL, 'file'), at: [0, 3] },
    { node: comp('s-src-kafka', 'kafka', TEAL, 'stream'), at: [0, 4] },
    { node: comp('s-src-delta', 'delta', TEAL, 'database'), at: [0, 5] },
  ]),
)
...
const sinks = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1, 1, 1], gap: G, padding: PT }, [
    { node: comp('s-sink-mem', 'memory', PURPLE, 'memory'), at: [0, 0] },
    { node: comp('s-sink-console', 'console', PURPLE, 'report'), at: [0, 1] },
    { node: comp('s-sink-file', 'file', PURPLE, 'file'), at: [0, 2] },
    { node: comp('s-sink-kafka', 'kafka', PURPLE, 'stream'), at: [0, 3] },
    { node: comp('s-sink-delta', 'delta', PURPLE, 'database'), at: [0, 4] },
    { node: comp('s-sink-fb', 'foreachBatch', PURPLE, 'copy'), at: [0, 5] },
    { node: comp('s-sink-fe', 'foreach', PURPLE, 'copy'), at: [0, 6] },
  ]),
)
...
const output = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1], gap: G, padding: PT }, [
    { node: comp('s-out-mem', 'in-memory view', TEAL, 'memory'), at: [0, 0] },
    { node: comp('s-out-console', 'console stdout', TEAL, 'report'), at: [0, 1] },
    { node: comp('s-out-files', 'Files', TEAL, 'file'), at: [0, 2] },
    { node: comp('s-out-kafka', 'Kafka topic', TEAL, 'stream'), at: [0, 3] },
    { node: comp('s-out-delta', 'Delta table', TEAL, 'database'), at: [0, 4] },
  ]),
)
