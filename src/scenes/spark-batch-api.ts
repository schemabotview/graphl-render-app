...
// The WHOLE Spark BATCH API on one 16:9 wall — ported from graphl-ux/src/scenes/
// spark-batch-api.ts (itself from NodeMap's `spark-batch.ts`). This is the shared
// MASTER MAP: modules 03–07, 09, 10 all ride THIS scene and just spotlight their own
// region via the manifest `focus`/`highlight`, so the learner builds one retained map.
// Same node ids as the source (e.g. `b-rdd-rbk`) so the module manifests' wiring transfers.
//
// Port transforms vs. graphl-ux (mirror the spark-architecture / databricks ports):
//   - `wgrid` imported from patterns.ts (graphl-ux inlined it).
//   - graphl-ux `kind: 'label'` leaves (the ~150 method names in the four dense API
//     lanes) become `term` chips — this engine's NodeKind has no bare 'label'. NOTE:
//     the source used bare labels precisely because ~150 FILLED chips read as clutter;
//     if the dense lanes look busy, the follow-up is to add a 'label' kind to the engine.
//   - I/O endpoint `comp()` nodes gain icons (file/json/database), matching spark-architecture.
...
/** A filled leaf chip — the text IS the value (`term`). Default outside the I/O endpoints. */
const chip = (id: string, label: string, color: string): NodeSeed => ({ id, label, color, kind: 'term', cell: [0, 0] })

/** Was graphl-ux `kind: 'label'` (bare text) — rendered here as a `term` chip (no 'label' kind). */
const lbl = (id: string, label: string, color: string): NodeSeed => chip(id, label, color)

/** A COMPONENT leaf — icon glyph + label (`symbol`). Used for the I/O endpoints (CSV, Parquet, …). */
...
const sources = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1, 1, 1, 1], gap: G, padding: PT }, [
    { node: comp('b-src-csv', 'CSV', TEAL, 'file'), at: [0, 0] },
    { node: comp('b-src-parquet', 'Parquet', TEAL, 'file'), at: [0, 1] },
    { node: comp('b-src-json', 'JSON', TEAL, 'json'), at: [0, 2] },
    { node: comp('b-src-orc', 'ORC', TEAL, 'file'), at: [0, 3] },
    { node: comp('b-src-jdbc', 'JDBC', TEAL, 'database'), at: [0, 4] },
    { node: comp('b-src-hive', 'Hive', TEAL, 'database'), at: [0, 5] },
    { node: comp('b-src-files', 'Files', TEAL, 'file'), at: [0, 6] },
    { node: comp('b-src-delta', 'Delta', TEAL, 'database'), at: [0, 7] },
  ]),
)
...
const output = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1], gap: G, padding: PT }, [
    { node: comp('b-out-files', 'Files', TEAL, 'file'), at: [0, 0] },
    { node: comp('b-out-hive', 'Hive table', TEAL, 'database'), at: [0, 1] },
    { node: comp('b-out-jdbc', 'JDBC target', TEAL, 'database'), at: [0, 2] },
    { node: comp('b-out-delta', 'Delta table', TEAL, 'database'), at: [0, 3] },
  ]),
)
